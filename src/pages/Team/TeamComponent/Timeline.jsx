    ...
    <div className="flex flex-col gap-6 ">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {executives.slice(0, firstRowCount).map((executive, index) => (
          <div key={index} className="relative group overflow-hidden rounded-lg" style={{ width: "200px", height: "200px" }}>
            <img
              src={executive.image || "https://media.istockphoto.com/id/1432561113/photo/contact-icon-3d-render-illustration.jpg?s=612x612&w=0&k=20&c=DnbTbyt9K8sxNF_Nb-PsEG1EaCDIgxB4_SyFe9Mblys="}
              alt={executive.name}
            ...
            <div className="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center p-4">
              <span className="text-white text-sm md:text-base font-medium">{executive.name}</span>
            </div>
          </div>
        ))}
      </div>
          ...
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div
              ...
              <div
                ...
                <div
                  className="h-4 w-4 rounded-full bg-gray-800 border border-gray-700 p-2" />
              </div>
              <h3
                className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold  text-yellow-500 opacity-60 ">
                {item.title}
              </h3>
            </div>
            ...
            <div className="relative pl-20  md:pl-0 w-full">
              <h3
                className="md:hidden block text-2xl mb-4 text-left font-bold  text-yellow-500 opacity-60">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
...
export function PastTimeline() {
    ...
    },
    {
      ...
      content: (
        <div>
          ...
          <ResponsiveExecutiveTeam executives={executives2022} firstRowCount={3} />
        </div>
      ),
    },
    {
  ...
  return (
    <div className="w-full ">
       <BackgroundBeamsWithCollision from="Team" className="absolute inset-0 w-full h-full" />
      <Spotlight/>
      <Timeline data={data} />
    </div>
  );
